...
const changeRecording = async (index) => {
  const { _id, date, where, howMany } = recordingsArray[index];
...
const editRecording = (index) => {
  const { date, where, howMany } = recordingsArray[index];
  inputWhereValue = where;
  inputHowManyValue = howMany;
  inputDateValue = date;
  ...
  editInputDate.id = 'input_edit_date';
  editInputDate.type = 'date';
  editInputDate.value = date;
  editInputDate.addEventListener('change', changeInputDate);
  ...
  activeRecording.appendChild(editInputDate);
...
const editTitleWhere = (index) => {
  ...
  newInputEditWhere.addEventListener('blur', (event) => {
...
const render = () => {
  ...
  recordingsArray.forEach((item, index) => {
    ...
    recordTitleDate.innerText = changeDateText(date);
